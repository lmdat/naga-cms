...
@section('content')
    <section class="content-header">
        <h1>
            {{ trans($lang_mod . '.news_edit_header') }}
            @if(session()->has('message-error'))
                <small><label class="label label-danger">Oh snap! {{ Session::get('message-error') }}</label></small>
            @endif

            @if(session()->has('message-success'))
                <small><label class="label label-success">Yeah! {{ Session::get('message-success') }}</label></small>
            @endif
        </h1>
        {{--<ol class="breadcrumb">--}}
    ...
    <section class="content">
        {!! Form::open(['url' => Request::url() . $qs, 'method'=>'put', 'name' => 'newsForm', 'id' => 'newsForm', 'role' => 'form']) !!}
        <div class="row">
            <div class="col-md-8">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="form-group @if($errors->has('cat_id')) has-error @endif">
                            ...
                            <select name="cat_id" id="cat_id" class="form-control input-sm">
                                ...
                                @foreach($tree_data as $item)
                                    @if($errors->any())
                                    <option value="{{$item->id}}" @if($item->parent_id == 0) disabled @endif @if($item->id == old('cat_id')) selected @endif>{{ $item->cat_name_tmp }}</option>
                                    @else
                                    <option value="{{$item->id}}" @if($item->parent_id == 0) disabled @endif @if($item->id == $news->cat_id) selected @endif>{{ $item->cat_name_tmp }}</option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @if($errors->has('title')) has-error @endif">
                            ...
                            {!! Form::text('title', $news->title, ['id'=>'title', 'class' => 'form-control input-sm', 'placeholder' => 'Required']) !!}
                            @if ($errors->has('title'))<p><small class="text-red">{!!$errors->first('title')!!}</small></p> @endif
                        </div>

                        <div class="form-group">
                            <label for="">{{ trans($lang_mod . '.alias') }}</label>
                            {!! Form::text('alias', $news->alias, ['id'=>'alias', 'class' => 'form-control input-sm', 'placeholder' => '']) !!}

                        </div>

                        <div class="form-group">
                            <label for="intro_content">{{ trans($lang_mod . '.intro_content') }}</label>
                            {!! Form::textarea('intro_content', $news->intro_content, ['class' => 'form-control intro-content-editor', 'placeholder' => '', 'id'=>'intro_content', 'style' => 'height: 90px']) !!}
                        </div>
                        ...
                        <div class="form-group @if($errors->has('main_content')) has-error @endif">
                            ...
                            {!! Form::textarea('main_content', $news->detail->main_content, ['class' => 'form-control main-content-editor', 'placeholder' => '', 'id'=>'main_content']) !!}
                            @if ($errors->has('main_content'))<p><small class="text-red">{!!$errors->first('main_content')!!}</small></p> @endif
                        </div>

                        {{--<div class="form-group">--}}
                            {{--{!! Form::text('cat_highlight_time_test', '', ['id'=>'cat_highlight_time_test', 'class' => 'form-control input-sm', 'placeholder' => '', ]) !!}--}}
                        {{--</div>--}}
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans($lang_mod . '.news_relations') }} <span id="total_relation">{{ $relations != null ? '(' . count($relations) . ')' : '' }}</span></h3>
                    </div>
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <a id="btn_trash" href="{{ url($prefix_url . '/trash', ['id'=>$news->id]) . $qs }}" class="btn btn-flat bg-red btn-sm pull-left"><i class="fa fa-trash"></i> {{ trans($lang_common . '.btn_trash') }}</a>
                                ...
                                <script>
                                    $(function(){
                                        $('#btn_trash').on('click', function(e){
                                            e.preventDefault();
                                            if(confirm('{{ trans($lang_mod . '.trash_message') }}')){
                        ...
                        <div class="form-group">
                            ...
                            <div id="status_form" class="row" style="display: none">
                                <div class="container-fluid">
                                    <div class="col-sm-7">
                                        {!! Form::select('status', $news_status, $news->status, ['class' => 'form-control input-sm', 'id' => 'status']) !!}
                                    </div>
                                    <div class="col-md-5">
                            ...
                            <script>
                                $(function(){
                                    ...
                                    $('#save_status_form').on('click', function(e){
                                        ...
                                        var _publish = {{ config('constant.NEWS_PUBLISHED_STATUS') }};
                                        var _auto_publish = {{ config('constant.NEWS_AUTO_PUBLISHED_STATUS') }};
                                        ...
                                        if($('#status').val() == _publish){
                                            $('#published_at_content').html('{{ trans($lang_mod . '.publish_now') }}');
                                            $('#edit_published_at').fadeOut();
                                        }
                                        else if($('#status').val() == _auto_publish){

                                            $('#published_at_content').html('@if($news->published_at == '') {{ trans($lang_mod . '.publish_now') }} @else {{ \App\Libs\Utils\Vii::formatDateTime($news->published_at, 'd/m/Y H:i') }} @endif');
                                            $('#edit_published_at').fadeIn();
                                        }
                                        else{
                                            $('#published_at_content').html('{{ trans($lang_common . '.not_answer') }}');
                                            $('#edit_published_at').fadeOut();
                                        }
                                    });
...
//                                        }, 500);
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <span id="hot_content">
                                        @if($errors->any())
                                            {{ old('is_hot') == 1 ? trans($lang_common . '.radio_yes') : trans($lang_common . '.radio_no') }}
                                        @else
                                            {{ $news->is_hot == 1 ? trans($lang_common . '.radio_yes') : trans($lang_common . '.radio_no') }}
                                        @endif
                                    </span>
                            ...
                            <div id="hot_form" class="row" style="display: none">
                                <div class="container-fluid">
                                    <div class="col-sm-7">
                                        {!! Form::checkbox('is_hot', 1, ($news->is_hot == 1), ['id' => 'is_hot', 'class'=>'form-control input-switch', 'data-size' => 'small', 'data-on-text'=>trans($lang_common . '.radio_yes'), 'data-off-text'=>trans($lang_common . '.radio_no')]) !!}
                                    </div>
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    @if($news->status == config('constant.NEWS_PUBLISHED_STATUS') || $news->status == config('constant.NEWS_AUTO_PUBLISHED_STATUS'))
                                        <span id="published_at_content">
                                            @if($errors->any())
                                                {{ old('published_at') }}
                                            @else
                                                {{ \App\Libs\Utils\Vii::formatDateTime($news->published_at, 'd/m/Y H:i') }}
                                            @endif
                                        </span>
                                        <a id="edit_published_at" href="#" data-published-at-form="#published_at_form"><i class="fa fa-edit"></i></a>
                                    @else
                                        <span id="published_at_content">{{ trans($lang_common . '.not_answer') }}</span>
                                        <a id="edit_published_at" href="#" data-published-at-form="#published_at_form" style="display: none"><i class="fa fa-edit"></i></a>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="container-fluid">
                                    <div class="col-md-7">
                                        ...
                                        <div class="input-group">
                                            ...
                                            {!! Form::text('published_at', App\Libs\Utils\Time::niceShort($news->published_at), ['id'=>'published_at', 'class' => 'form-control input-sm', 'placeholder' => 'DD/MM/YYYY HH:mm']) !!}
                                        </div>
                                    </div>
                    ...
                    <div class="box-footer">
                        ...
                        <a href="{{ url($prefix_url ) . $qs }}" class="btn btn-flat bg-orange btn-sm pull-left"><i class="fa fa-reply"></i> {{ trans($lang_common . '.btn_exit') }}</a>
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <span id="highlight_content">
                                        @if($errors->any())
                                            {{ old('is_highlight') == 1 ? trans($lang_common . '.radio_yes') : trans($lang_common . '.radio_no') }}
                                        @else
                                            {{ $news->is_highlight == 1 ? trans($lang_common . '.radio_yes') : trans($lang_common . '.radio_no') }}
                                        @endif

                                    </span>
                                    <a id="edit_highlight" href="#" data-highlight-form="#highlight_form" @if($news->is_hot == 1) style="display: none" @endif><i class="fa fa-edit"></i></a>
                                </div>
                            </div>
                            ...
                            <div id="highlight_form" class="row" style="display: none">
                                <div class="container-fluid">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            {!! Form::checkbox('is_highlight', 1, ($news->is_highlight == 1), ['id' => 'is_highlight', 'class'=>'form-control input-switch', 'data-size' => 'small', 'data-on-text'=>trans($lang_common . '.radio_yes'), 'data-off-text'=>trans($lang_common . '.radio_no')]) !!}
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <?php
                                                    $home_hl_time = '';
                                                    if($news->is_highlight == 1 && $news->home_highlight != null){
                                                        if($news->home_highlight->hl_start_time != ''
                                                                && $news->home_highlight->hl_end_time != ''){
                                                            $home_hl_time = \App\Libs\Utils\Vii::formatDateTime(strval($news->home_highlight->hl_start_time), 'd/m/Y H:i');
                                                            $home_hl_time .= ' - ' . \App\Libs\Utils\Vii::formatDateTime(strval($news->home_highlight->hl_end_time), 'd/m/Y H:i');
                                                        }

                                                    }
                                                ?>
                                                {!! Form::text('highlight_time', $home_hl_time, ['id'=>'highlight_time', 'class' => 'form-control input-sm', 'placeholder' => '', 'disabled' => ($news->is_highlight == 0 ? 'disabled' : null)]) !!}
                                            </div>
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <span id="cat_highlight_content">
                                        @if($errors->any())
                                            {{ old('is_cat_highlight') == 1 ? trans($lang_common . '.radio_yes') : trans($lang_common . '.radio_no') }}
                                        @else
                                            {{ $news->is_cat_highlight == 1 ? trans($lang_common . '.radio_yes') : trans($lang_common . '.radio_no') }}
                                        @endif
                            ...
                            <div id="cat_highlight_form" class="row" style="display: none">
                                <div class="container-fluid">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            {!! Form::checkbox('is_cat_highlight', 1, ($news->is_cat_highlight == 1), ['id' => 'is_cat_highlight', 'class'=>'form-control input-switch', 'data-size' => 'small', 'data-on-text'=>trans($lang_common . '.radio_yes'), 'data-off-text'=>trans($lang_common . '.radio_no')]) !!}
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <?php
                                                $cat_hl_time = '';
                                                if($news->is_cat_highlight == 1 && $news->cat_highlight != null){
                                                    if($news->cat_highlight->hl_start_time != ''
                                                            && $news->cat_highlight->hl_end_time != ''){
                                                        $cat_hl_time = \App\Libs\Utils\Vii::formatDateTime(strval($news->cat_highlight->hl_start_time), 'd/m/Y H:i');
                                                        $cat_hl_time .= ' - ' . \App\Libs\Utils\Vii::formatDateTime(strval($news->cat_highlight->hl_end_time), 'd/m/Y H:i');
                                                    }

                                                }
                                                ?>
                                                {!! Form::text('cat_highlight_time', $cat_hl_time, ['id'=>'cat_highlight_time', 'class' => 'form-control input-sm', 'placeholder' => '', 'disabled' => ($news->is_cat_highlight == 0 ? 'disabled' : null)]) !!}
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        @foreach($news_type as $k => $type)
                            <div class="checkbox">
                                <label>
                                    {!! Form::radio('news_type', $k, ($k == $news->news_type), ['id' => 'news_type_' . $k]) !!} <i class="{{ $type[0] }}"></i> <strong>{{ $type[1] }}</strong>
                                </label>
                            </div>
                        @endforeach
                        </div>
                    ...
                    <div class="box-body">
                        <div id='thumbnail_featured_image' class="thumbnail" style='display: ;'>
                            <?php
                                ...
                                $img_src = asset('media/local/img/cover_featured_image.png');
                                if($news->featured_image != ''){
                                    $img_src = $news->featured_image;
                                }
                            ?>
                            <img src="{{ $img_src }}" data-unique-id="2" style="zoom: 70%"/>
                            {!! Form::hidden('featured_image', '', ['id'=>'featured_image']) !!}
                        </div>
                        <button id="select_featured_image" type="button" class="btn btn-xs bg-maroon" data-unique-id="2"><i class="fa fa-photo"></i> {{ trans($lang_common . '.btn_select_image') }}</button>
                        <button id="clear_featured_image" type="button" class="btn btn-xs bg-orange pull-right" @if($news->featured_image == '') style='display: none;' @endif ><i class="fa fa-remove"></i> {{ trans($lang_common . '.btn_delete') }}</button>
                    </div>
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            {!! Form::text('tags', $news->tags, ['id'=>'tags', 'class' => 'form-control input-sm', 'placeholder' => '']) !!}
                        </div>
                        <script>
                            $(function(){
                                ...
                                var rids = [{{ $news_relation_ids }}];

                                initDeleteRelationItem(rids);
                            ...
                            function initDeleteRelationItem(rids){
                                $('#news_relations').find('button.btn-delete-relations').on('click', function(){
                                ...
                                <div class="tab-pane fade in active" id="general">
                                    ...
                                    <div content="col-md-12">
                                        <div class="form-group">
                                            ...
                                            {!! Form::text('meta_title', isset($news->meta_data->meta_title) ? $news->meta_data->meta_title : '', ['id'=>'meta_title', 'class' => 'form-control input-sm', 'placeholder' => (@$news->meta_data->meta_title == '') ? $news->title : @$news->meta_data->meta_title]) !!}
                                        </div>

                                        <div class="form-group">
                                            <label>{{ trans($lang_mod . '.meta_keywords') }}</label>
                                            {!! Form::text('meta_keywords', @$news->meta_data->meta_keywords, ['id'=>'meta_keywords', 'class' => 'form-control input-sm', 'placeholder' => '']) !!}
                                        </div>

                                        <div class="form-group">
                                            <label>{{ trans($lang_mod . '.meta_description') }}</label>
                                            {!! Form::textarea('meta_description', @$news->meta_data->meta_description, ['id'=>'meta_description', 'class' => 'form-control input-sm', 'rows' => 3, 'placeholder' => '']) !!}
                                        </div>
                                ...
                                <div class="tab-pane fade in" id="social">
                                    ...
                                    <div content="col-md-12">
                                        <div class="form-group">
                                            ...
                                            {!! Form::text('og_title', @$news->meta_data->og_title, ['id'=>'og_title', 'class' => 'form-control input-sm', 'placeholder' => '']) !!}
                                        </div>

                                        <div class="form-group">
                                            <label>{{ trans($lang_mod . '.og_description') }}</label>
                                            {!! Form::textarea('og_description', @$news->meta_data->og_description, ['id'=>'og_description', 'class' => 'form-control input-sm', 'rows' => 3, 'placeholder' => '']) !!}
                                        </div>

                                        <div class="form-group">
                                            <label>{{ trans($lang_mod . '.og_image') }}</label>
                                            {!! Form::text('og_image', @$news->meta_data->og_image, ['id'=>'og_image', 'class' => 'form-control input-sm', 'rows' => 3, 'placeholder' => '']) !!}
                                        </div>
    ...
    <script>
        ...
        $(function(){
            ...
            $('input[name="cat_highlight_time"], input[name="highlight_time"]').daterangepicker({
                ...
                locale:{
                    format: 'DD/MM/YYYY HH:mm',
                    cancelLabel: '{{ trans($lang_common . '.btn_cancel') }}',
            ...
            $("input[name='file_uploader']").ViiUploader({
                ...
                getImagesUrl        :   '{{ url($prefix_url . '/media-list', [$news->id]) }}',
                //csrfMetaName      :   '',
                coverUpload         :   '{{asset('media/local/img/cover-upload.png')}}',
                btnOpenModalClass   :   'btn btn-xs btn-success btn-flat',
                btnOpenModalText    :   '<i class="fa fa-photo"></i> {{ trans($lang_common . '.btn_insert_media') }}',

...
@section('content')
    <section class="content-header">
        <h1>
            {{ trans($lang_mod . '.position_create_header') }}
            <small></small>
        </h1>
        <!--ol class="breadcrumb">
    ...
    <section class="content">
        {!! Form::open(['url' => Request::url() . $qs, 'name' => 'posForm', 'id' => 'posForm', 'role' => 'form']) !!}
        <div class="row">
            <div class="col-md-4">
                            ...
                            <div class="col-md-12">
                                <fieldset>
                                    ...
                                    @foreach(config('constant.ADS_POSITION.HOME_FIX') as $k => $v)
                                        <button type="button" name="btn_{{ str_replace('.', '_', $v) }}" id="btn_{{ str_replace('.', '_', $v) }}" class="btn btn-flat bg-olive btn-xs btn-block home-fix">{{ $v }}</button>
                                    @endforeach

                                    @foreach(config('constant.ADS_POSITION.HOME_FIX') as $k => $v)
                                        <input type="hidden" name="ads_position[]" id="{{ str_replace('.', '_', $v) }}" value="{{ $v }}|1|1"/>
                                    @endforeach
                            ...
                            <div class="col-md-12">
                                ...
                                <script>
                                    ...
                                    $(function(){
                                        ...
                                        add_ads_block_button(total_cat, $('input[class="cat-group"]:checked:first').val());
                            ...
                            <div class="col-md-12">
                                <fieldset>
                                    ...
                                    @foreach(config('constant.ADS_POSITION.CATEGORY_FIX') as $k => $v)
                                        <button type="button" name="btn_{{ str_replace('.', '_', $v) }}" id="btn_{{ str_replace('.', '_', $v) }}" class="btn btn-flat bg-olive btn-xs btn-block category-fix">{{ $v }}</button>
                                    @endforeach
                            ...
                            <div class="col-md-12">
                                <fieldset>
                                    ...
                                    @foreach(config('constant.ADS_POSITION.DETAIL_NEWS_FIX') as $k => $v)
                                        <button type="button" name="btn_{{ str_replace('.', '_', $v) }}" id="btn_{{ str_replace('.', '_', $v) }}" class="btn btn-flat bg-olive btn-xs btn-block detail-news-fix">{{ $v }}</button>
                                    @endforeach
                            ...
                            <div class="col-md-12">
                                <fieldset>
                                    ...
                                    @foreach(config('constant.ADS_POSITION.DETAIL_VIDEO_FIX') as $k => $v)
                                        <button type="button" name="btn_{{ str_replace('.', '_', $v) }}" id="btn_{{ str_replace('.', '_', $v) }}" class="btn btn-flat bg-olive btn-xs btn-block detail-video-fix">{{ $v }}</button>
                                    @endforeach
